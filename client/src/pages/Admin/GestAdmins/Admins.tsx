...
import AddPompiste from './AddAdmin';
...
const Admins: React.FC = () => {
  ...
  const { admins, isLoading } = useSelector((state: any) => state.admins);
  ...
  const handleSelectAllChange = (e: { target: { checked: any } }) => {
    ...
    setSelectedIds(isChecked ? admins.map((pompiste: { _id: any }) => pompiste._id) : []);
  };
  ...
  return (
   ...
   <h1 className="text-4xl font-bold mb-8"> Gestion des Admins</h1>
      {isLoading ? (
       <Spinner/>
      ) : (
          ...
          <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
              <div className="flex-1 flex items-center space-x-2">
                <h5>
                  <span className="dark:text-white">Tous les admins : </span>
                  <span className="dark:text-white"> {admins.length}</span>
                </h5>
                ...
                {filteredAdmins?.map((admin :any ) => (
                  <tr key={admin._id} className="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                    <td className="p-4 w-4">
                      ...
                      <input
                        type="checkbox"
                        id={`checkbox${admin._id}`}
                        name="options[]"
                        value={admin._id}
                        checked={selectAllChecked || selectedIds.includes(admin._id)}
                        onChange={(e) => handleCheckboxChange(e, admin._id)}
                      />
        
                    <label htmlFor={`checkbox-table-search-${admin._id}`} className="sr-only">
                      Select
                    </label>
                    ...
                    <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                     ...
                     <img 
                      src={admin?.image?.buffer ? `data:image/png;base64,${admin.image.buffer}` : defaultUser}
