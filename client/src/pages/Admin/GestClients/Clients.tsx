...
import ViewClient from './ViewClientModal';
...
const Client : React.FC = () => {
  ...
  const handleSelectAllChange = (e: { target: { checked: any; }; }) => {
    ...
    setSelectedIds(isChecked ? clients.map((pompiste: { _id: any; }) => pompiste._id) : []);
  };
...
<h1 className="text-4xl font-bold mb-8"> Gestion des Clients</h1>
{
  isLoading ? (
    <Spinner/>
  ) : (
      ...
      <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
          <div className="flex-1 flex items-center space-x-2">
            <h5>
            <span className="dark:text-white text-black">Tous les Clients :</span>
              <span className="dark:text-white"> {clients.length}</span>
            </h5>
              ...
              <tr key={client._id} className="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                <td className="p-4 w-4">
                  ...
                  <input
                    ...
                    name="options[]"
                    value={client._id}
                    checked={selectAllChecked || selectedIds.includes(client._id)}
                ...
                <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                 ...
                 <img 
                  src={client?.image?.buffer ? `data:image/png;base64,${client.image.buffer}` : defaultUser}
                  alt="img" width={50} height={50}
                  className='rounded-full' />
                  </div>
                </th>
                <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{client.username}</td>
                <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{client.CIN}</td>
                <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{client.email}</td>
                <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{client.phone}</td>
                <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                  <div className="flex items-center justify-center space-x-4">
                    <button  onClick={() =>{setElement(client) , setIsEditModalOpen(true)}} type="button" data-modal-target="delete-modal" data-modal-toggle="delete-modal" className="flex items-center text-yellow-700 hover:text-white border border-yellow-700 hover:bg-yellow-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-2 py-2 text-center dark:border-yellow-500 dark:text-yellow-500 dark:hover:text-white dark:hover:bg-yellow-600 dark:focus:ring-yellow-900">
  ...
  <Destroy show={isDestroyModalOpen} handleClose={() => setIsDestroyModalOpen(false)} ids={selectedIds} EndPoint="/clients/destroy" onDestroySuccess={fetchClients} />
  <Delete show={isshowDeleteModalOpen} handleClose={() => setIsDeleteModalOpen(false)} Id={selectedId} EndPoint="/clients"  onDeletionSuccess={fetchClients}/>
  <EditClient show={isEditModalOpen} handleClose={() => setIsEditModalOpen(false)} Element={Element} />
  <ViewClient show={isViewModalOpen} handleClose={() => setIsViewModalOpen(false)}  Element={Element}/>
  <AddClient show={isAddModalOpen} handleClose={() => setIsAddModalOpen(false)} />   
