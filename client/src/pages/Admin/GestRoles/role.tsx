...
const Role: React.FC = () => {
  ...
  const handleSelectAllChange = (e: { target: { checked: any } }) => {
    ...
    setSelectedIds(isChecked ? roles.map((role: { _id: any }) => role._id) : []);
  };
  ...
  return (
    <div>
          ...
          <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
              <div className="flex-1 flex items-center space-x-2">
                <h5>
                    <span className="dark:text-white">Tous les rôles : </span>
                ...
                <table className="w-full text-sm  text-gray-500 dark:text-gray-400 text-center  ">
                  ...
                  {roles && Array.isArray(roles) && roles.map((role) => (
                      ...
                      >
                        <td className="p-4 w-4">
                          <div className="flex items-center">
                            <input
                              ...
                              name="options[]"
                              value={role._id}
                              checked={selectAllChecked || selectedIds.includes(role._id)}
                        ...
                        <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{role.permissions.length}</td>
      ...
      <ViewRole show={isViewModalOpen} handleClose={() => setIsViewModalOpen(false)} Element={Element} />
      <AddRole show={isAddModalOpen} handleClose={() => setIsAddModalOpen(false)} />
    </div>
  );
};
